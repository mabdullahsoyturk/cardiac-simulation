...
__global__ void kernel1_pde(double* E, double* E_prev, double* R, const double alpha, const int n, const int m,
  ...
  if (column_index <= n && row_index <= m) {
    E[row_index * (n + 2) + column_index] =
        E_prev[row_index * (n + 2) + column_index] +
        alpha * (E_prev[row_index * (n + 2) + column_index + 1] + E_prev[row_index * (n + 2) + column_index - 1] -
                 4 * E_prev[row_index * (n + 2) + column_index] + E_prev[(row_index + 1) * (n + 2) + column_index] +
...
__global__ void kernel1_ode(double* E, double* E_prev, double* R, const double alpha, const int n, const int m,
  ...
  if (column_index <= n && row_index <= m) {
    E[row_index * (n + 2) + column_index] =
        E[row_index * (n + 2) + column_index] -
        dt * (kk * E[row_index * (n + 2) + column_index] * (E[row_index * (n + 2) + column_index] - a) *
                  (E[row_index * (n + 2) + column_index] - 1) +
              E[row_index * (n + 2) + column_index] * R[row_index * (n + 2) + column_index]);

    R[row_index * (n + 2) + column_index] =
        R[row_index * (n + 2) + column_index] +
        dt * (epsilon + M1 * R[row_index * (n + 2) + column_index] / (E[row_index * (n + 2) + column_index] + M2)) *
            (-R[row_index * (n + 2) + column_index] -
             kk * E[row_index * (n + 2) + column_index] * (E[row_index * (n + 2) + column_index] - b - 1));
  }
}
...
__global__ void kernel4(double* E, double* E_prev, double* R, const double alpha, const int n, const int m,
  ...
  if (row_index <= m && threadIdx.x == 0) {
    ...
    E_prev[row_index * (n + 2)] = temp;
    E_prev[row_index * (n + 2) + n + 1] = temp2;
  }
  ...
  if (column_index <= n && threadIdx.y == 0) {
    ...
    E_prev[(m + 1) * (n + 2) + column_index] = temp2;
  }
